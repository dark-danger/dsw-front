...
export const StudentDashboardPage: React.FC = () => {
  ...
  return (
    <div className="space-y-6">
      {/* Welcome Card */}
      <div className="glass-panel p-6 bg-gradient-to-r from-black via-[#06140d] to-black border-l-4 border-l-emerald-500 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          <h2 className="text-2xl font-bold text-white">Welcome, {user?.name}!</h2>
          <p className="text-xs text-slate-300 mt-1">Roll No: {user?.roll_number} • Course: {user?.course_branch} ({user?.year})</p>
        </div>

        {myRank && (
          <div className="flex items-center gap-4 bg-black/80 p-3 rounded-2xl border border-white/10 shrink-0">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <Link to="/student/leaderboard-tasks" className="glass-card p-6 flex items-center justify-between group hover:border-emerald-500/50">
          <div>
            ...
            <p className="text-xs text-slate-300 mt-1">Complete volunteer duties & event tasks to earn points for the university leaderboard.</p>
          </div>
        </Link>
        ...
        <Link to="/student/queries" className="glass-card p-6 flex items-center justify-between group hover:border-teal-500/50">
          <div>
            ...
            <p className="text-xs text-slate-300 mt-1">Directly contact the Dean of Student Welfare office for assistance or inquiry.</p>
          </div>
        </Link>
      </div>
    </div>
